...
const args = <ICLIOpts>(<any>yargs
  ...
  .describe(
    'reporters',
    'List of reporters to use, can also be path to custom file, available are: `pretty` (stderr) and `json` (stdout)',
  )
  ...
  .coerce('attributePattern', (str: string) => new RegExp(str))
  ...
  .coerce('templateMatcher', (str: string) => new RegExp(str))
  ...
  .coerce('assumeTextCondition', (str: string) => new RegExp(str))
  .describe(
    'assumeTextCondition',
    'Pattern to identify minimal text that should force a translation to be present.',
  )

  .string('urlRegEx')
  .alias('urlRegEx', 'u')
  .default('urlRegEx', I18nValidator.urlRegEx.source)
  .coerce('urlRegEx', (str: string) => new RegExp(str))
  .describe('urlRegEx', 'Pattern to identify as url and ignore.')

  .string('emailRegEx')
  .alias('emailRegEx', 'e')
  .default('emailRegEx', I18nValidator.emailRegEx.source)
  .coerce('emailRegEx', (str: string) => new RegExp(str))
  .describe('emailRegEx', 'Pattern to identify as email and ignore.').argv);
...
const reporters: IReporter[] = args.reporters.map(nameOrFile => {
  ...
  if (existsSync(potentialFilePath)) {
    return new (<{ new(): IReporter }>require(potentialFilePath))(); // tslint:disable-line non-literal-require
  }
