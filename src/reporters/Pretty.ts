...
export class Pretty implements IReporter {
    public async report(problems: IProblem[]): Promise<void> {
        ...
        problems.forEach(problem => {
            const prettyFile = `${green(problem.fileName.replace(`${process.cwd()}/`, ''))}:${problem.line}`;
            switch (problem.problem) {
                ...
                case 'format':
                    ...
                    break;
                case 'nested':
                    console.error(`${prettyFile}: Parent already already has attribute \`${blue(problem.meta)}\``);
                    break;
                default:
